...
function activateCustomSearch(module) {
	...
	if (iterator.length == 0) {
		ldsPrompt.show(alert_arr['ERROR'], alert_arr.SELECTCONDITION);
		return false;
	}
		...
		if (p1 != null && p1 != undefined) {
			...
			if (p4.value == 'date' || p4.value == 'datetime') {
				...
				if (d1 != null && d1 != undefined && d1.value != '' && d2 != null && d2 != undefined && d2.value != '') {
					...
					ldsPrompt.show(alert_arr['ERROR'], alert_arr.ERR_INVALID_DATE);
					return false;
				}
			}
		}
...
function mergeMassEditRecords(selectedNames, obj, divid, module) {
	var idstring = getMergeRecords(selectedNames, 100, 1);
	if (idstring !== false) {
